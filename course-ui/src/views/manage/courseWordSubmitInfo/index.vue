    ...
    <el-table v-loading="loading" :data="courseWordSubmitInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="编号" align="center" prop="id" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="作业" align="center" prop="wordId" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="提交内容" align="center" prop="submitContent" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="提交时间" align="center" prop="submitTime" width="180" v-if="columns[6].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                ...
                <el-table-column label="分数" align="center" prop="score" v-if="columns[8].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="批阅时间" align="center" prop="reviewTime" width="180" v-if="columns[9].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.reviewTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="老师" align="center" prop="teacherId" v-if="columns[10].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="学生" align="center" prop="userId" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:courseWordSubmitInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:courseWordSubmitInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="CourseWordSubmitInfo">
  import { listCourseWordSubmitInfo, getCourseWordSubmitInfo, delCourseWordSubmitInfo, addCourseWordSubmitInfo, updateCourseWordSubmitInfo } from "@/api/manage/courseWordSubmitInfo";

  const { proxy } = getCurrentInstance();
      const { course_word_submit_status, course_word_submit_review_status } = proxy.useDict('course_word_submit_status', 'course_word_submit_review_status');
